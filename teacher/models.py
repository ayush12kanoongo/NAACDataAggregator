...
class Teacher(models.Model):
    ...
    pan_no= models.pan_no= models.CharField(max_length=10,null=True,blank=True)
    aadhar_no = models.CharField(max_length=12, null=True, blank=True)
    prog = models.ForeignKey(Program, null=True, blank=True, on_delete=models.SET_NULL)
...
class Book_research_published(models.Model):    #3.4.3, 3.4.4
    ...
    def __str__(self):
        return (str(self.teacher)+'@'+self.publish_title)
...
class Teacher_collaborative_activity_participation(models.Model):   #3.7.1
    ...
    teacher = models.ForeignKey(Teacher, on_delete=models.CASCADE)
    def __str__(self):
        return (str(self.teacher) + '@' + str(self.program_title))
